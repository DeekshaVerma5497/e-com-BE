...
public class WishlistService {
    ...
    public Wishlist renameWishlist(String userId, Long wishlistId, String newName) {
        Wishlist w = wishlistRepo.findById(wishlistId)
            .filter(ws -> ws.getUser().getUserId().equals(userId))
            .orElseThrow(() ->
                new ResourceNotFoundException("Wishlist","id",wishlistId.toString())
            );
        w.setName(newName);
        return wishlistRepo.save(w);
    }
    ...
    public Wishlist addItem(String userId, Long wishlistId, String productCode) {
        ...
        var existing = itemRepo.findByWishlist_WishlistIdAndProduct_ProductCode(
            wishlistId, productCode
        ).orElse(null);
    ...
    public Wishlist removeItem(String userId, Long wishlistId, String productCode) {
        WishlistItem item = itemRepo.findByWishlist_WishlistIdAndProduct_ProductCode(
            wishlistId, productCode
        ).filter(i -> i.getWishlist().getUser().getUserId().equals(userId))
         .orElseThrow(() ->
             new ResourceNotFoundException("WishlistItem","productCode",productCode)
         );
